...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('medical_records', function (Blueprint $table) {
            ...
            $table->index(['tenant_id', 'clinic_id', 'patient_id', 'finalized_at'], 'medical_records_patient_history_index');
            $table->foreign(['tenant_id', 'clinic_id'])
                ->references(['tenant_id', 'id'])->on('clinics')->cascadeOnDelete();
            $table->foreign(['tenant_id', 'clinic_id', 'encounter_id'], 'medical_records_encounter_fk')
                ->references(['tenant_id', 'clinic_id', 'id'])->on('encounters')->restrictOnDelete();
            $table->foreign(['tenant_id', 'patient_id'], 'medical_records_patient_fk')
                ->references(['tenant_id', 'id'])->on('patients')->restrictOnDelete();
            $table->foreign(['clinic_id', 'practitioner_id'], 'medical_records_practitioner_fk')
