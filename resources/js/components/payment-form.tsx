...
export function PaymentForm({
    ...
    const insufficientCash =
        cashTotal > 0 &&
        cashReceived !== '' &&
        (!Number.isSafeInteger(Number(cashReceived)) ||
            Number(cashReceived) < cashTotal);
    const valid =
        !invalidRows && total > 0 && remaining >= 0 && !insufficientCash;
        ...
        >
            <div className="bg-muted/30 border-b p-4 md:p-5">
                ...
                <p className="text-muted-foreground mt-2 text-xs">
                    Bayar penuh, sebagian, atau dengan beberapa metode.
                </p>
            </div>
            <form
                ...
                onSubmit={(event) => {
                    ...
                    form.post(store.url(invoice.uuid), {
                        preserveScroll: true,
                        onSuccess: (page) => {
                            const updated = page.props as unknown as {
                                invoice: BillingInvoice;
                                paymentToken: string;
                            };
            ...
            >
                ...
                {rows.length > 1 && remaining === 0 && invalidRows && (
                    <p className="text-muted-foreground text-xs">
                        Bagi nominal pada setiap metode sesuai pembayaran
                        pasien.
                    </p>
                )}
                {cashTotal > 0 && (
                    <div className="bg-muted/30 grid gap-2 rounded-lg border p-3">
                        <FormField
                            id="cash-received"
                            label="Uang tunai diterima (Rp, opsional)"
                        >
                            <Input
                                id="cash-received"
                                type="number"
                        ...
                        {insufficientCash && (
                            ...
                            >
                                Uang tunai kurang dari nominal pembayaran tunai.
                            </p>
                        )}
                        <p className="text-muted-foreground text-xs">
                            Alat bantu hitung kembalian; tagihan dicatat sesuai
                            nominal pembayaran.
                        </p>
                    </div>
                )}
                ...
                {remaining < 0 && (
                    <p role="alert" className="text-destructive text-xs">
                        Nominal melebihi sisa tagihan sebesar{' '}
                ...
                >
                    ...
                    {form.processing
                        ...
                        : remaining === 0
                          ...
                          : 'Bayar sebagian · ' + formatCurrency(total)}
                </Button>
            </form>
        </section>
    );
