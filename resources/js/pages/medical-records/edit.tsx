                    ...
                    <main className="grid min-w-0 gap-5">
                        {can.start && (
                            <section className="border-primary/30 bg-primary/5 rounded-xl border p-4">
                                ...
                                <p className="text-muted-foreground mt-1 text-sm">
                                    Mulai pemeriksaan untuk membuka form rekam
                                    medis.
                                </p>
                                <Button
                                    className="mt-4"
                        ...
                        >
                            ...
                            <TextAreaField
                                ...
                                onChange={(value) =>
                                    form.setData('prescription_notes', value)
                                }
                                disabled={!can.save || locked}
                        ...
                        >
                            <TextAreaField
                                ...
                                onChange={(value) =>
                                    form.setData('additional_notes', value)
                                }
                                disabled={!can.save || locked}
    ...
    useEffect(() => {
        ...
        const timer = window.setTimeout(async () => {
            ...
            try {
                ...
                setItems(
                    data.items.filter((item) => !exclude.includes(item.uuid)),
                );
            } catch (reason) {
    ...
    return (
        <div className="bg-muted/20 grid gap-4 rounded-lg border p-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <FormField id={`dose-${index}`} label="Dosis">
                    <Input
                        id={`dose-${index}`}
                ...
                <FormField id={`frequency-${index}`} label="Frekuensi">
                    <Input
                        id={`frequency-${index}`}
                ...
                <FormField id={`timing-${index}`} label="Waktu">
                    <Input
                        id={`timing-${index}`}
            ...
            <section className="bg-card rounded-xl border p-4">
                ...
                {previousEncounters.length === 0 ? (
                    <p className="text-muted-foreground mt-3 text-xs">
                        Belum ada RME final sebelumnya.
                    </p>
                ) : (
                    <div className="mt-3 grid gap-3">
                        {previousEncounters.map((previous) => (
                            <article
                                key={`${previous.date}-${previous.doctor}`}
                                className="border-l-primary border-l-2 pl-3"
                            >
